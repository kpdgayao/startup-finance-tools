  ...
  return (
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        ...
        className={cn(
          // border-rule-strong (not border-border): the popover surface is
          // identical to --card in both themes, so a select opened over its
          // own containing card has zero fill contrast — separation depends
          // entirely on this border. --rule-strong exists for exactly this;
          // see docs/superpowers/specs/2026-07-29-editorial-redesign-phase-1-design.md
          // and task-8-report.md ("select dropdown separation in dark mode").
          "bg-popover text-popover-foreground data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 relative z-50 max-h-(--radix-select-content-available-height) min-w-[8rem] origin-(--radix-select-content-transform-origin) overflow-x-hidden overflow-y-auto rounded-md border border-rule-strong",
          position === "popper" &&
